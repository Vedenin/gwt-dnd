 ...
 * the License.
 */
package com.github.vedenin.gwt.dnd.client;
...
import com.github.vedenin.gwt.dnd.client.util.*;
...
public class PickupDragController extends AbstractDragController {
     ...
     *                                     https://github.com/fredsa/gwt_dnd/wiki/GettingStarted) if entire document body is to be
     *                                     the boundary
    ...
    public void dragMove() {
        ...
        int scrollLeft = context.draggable.getParent() != null ? Math.round(context.draggable.getParent().getElement().getScrollLeft()) : 0;
        int scrollTop = context.draggable.getParent() != null ? Math.round(context.draggable.getParent().getElement().getScrollTop()) : 0;
        ...
        if (getBehaviorConstrainedToBoundaryPanel()) {
            desiredLeft = Math.max(0,
                    Math.min(desiredLeft, dropTargetClientWidth - context.draggable.getOffsetWidth())) + scrollLeft;
            desiredTop = Math.max(0,
                    Math.min(desiredTop, dropTargetClientHeight - context.draggable.getOffsetHeight())) + scrollTop;
        }
